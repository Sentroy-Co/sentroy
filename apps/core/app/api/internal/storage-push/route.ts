...
/**
 * Server-to-server push fan-out for Storage share notifications. `dispatchToUsers`
 * (web VAPID + APNs + FCM) lives in the core app, so the storage app POSTs here
 * (x-internal-secret) to reach a user's devices. Mirrors the mail/meet push path.
 * Also writes a `mail_push_events` row per user for the Electron desktop poll
 * bridge (VAPID doesn't work in Electron's Chromium).
 */
export async function POST(request: NextRequest) {
  ...
  const tag = typeof body.tag === "string" && body.tag ? body.tag : "storage-share"

  if (userIds.length === 0 || !title || !bodyText) {
    return jsonError("userIds, title, body required")
  }
