...
// Rewrites — linear subdomain'inde tanımlı olmayan ortak endpoint'leri
// (auth, company list/detail, avatar) core'a forward et.
const coreUrl = (
...
if (process.env.NODE_ENV === "production") {
  if (!process.env.CORE_APP_URL && !process.env.NEXT_PUBLIC_CORE_APP_URL) {
    console.warn(
      "⚠ [linear/next.config] Neither CORE_APP_URL nor NEXT_PUBLIC_CORE_APP_URL is set — auth + cross-app rewrites will hit localhost:3000 and fail with ECONNREFUSED. Set one in Coolify env.",
    )
  }
}
...
const nextConfig: NextConfig = {
  ...
  async rewrites() {
    return [
      ...
      // Company list + detail core'da; linear alt route'ları lokalde kalır.
      {
