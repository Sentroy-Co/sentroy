...
export async function GET(
  ...
  // Members sayisi — DB'den
  try {
    const members = await companyMemberModel.findByCompany(company.id)
    usage.members = members.length
  } catch {}

  // Storage usage — mail (company.mailStorageUsed) + buckets (cdn dosyaları).
  // İkisi aynı plan.storageLimit havuzunu paylaşır; UI tek bar gösterdiği için
  // toplamı tek değerde dönüyoruz. mailStorageUsed bağımsız da `mailStorageUsed`
  // alanından okunabilir, ihtiyaç olursa.
  try {
  ...
  // Domains ve mailboxes sayisi — Sentroy API'den
